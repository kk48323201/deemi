...
class Wechat_model extends CI_Model{
	...
	function __construct(){
		...
		$this->appsecret = isset($wxconfig['appsecret'])?$wxconfig['appsecret']:'';
		$this->encodingAesKey = isset($wxconfig['encodingAesKey'])?$wxconfig['encodingAesKey']:'';
		$this->token = isset($wxconfig['token'])?$wxconfig['token']:'';
		$this->debug = false;
	...
	public function sendCustomMessage($data){
		if (!$this->getAccessToken() && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$this->getAccessToken(),self::json_encode($data));
		if ($result)
		{
	...
	public function getAccessToken(){
		...
		$this->db->reconnect();
		$sql = "select AccessToken from dm_wechat_AccessToken where CreateTime > DATE_SUB(NOW(),INTERVAL  1 HOUR)";
		$query = $this->db->query($sql);
		$tmpData = $query->row_array();
		if(isset($tmpData['AccessToken'])){
			return $tmpData['AccessToken'];
		}else{
			$ch = curl_init();
			...
			$this->Data_model->addData(array('AccessToken'=>$access_token,'CreateTime'=>date("Y-m-d H:i:s")),'wechat_AccessToken');
			return $access_token;
		}
		/*$access_token = beiguo_get($key);
		if(!$access_token){
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE); 
			...
			$tmp = json_decode($output, true);
			$access_token = $tmp['access_token'];
			beiguo_save($access_token,$key,$access_token);
		}
		return $access_token;*/
	}
	...
	function jumpWechatOauth($url){
		$scope='snsapi_userinfo';//需要授权
		$state = '1';
		$oauth_url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->appid.'&redirect_uri='.urlencode($url).'&response_type=code&scope='.$scope.'&state=1#wechat_redirect';
		header("Location:".$oauth_url);exit;
	}
	...
	function sendTemplateMsg($touser, $template_id, $data, $url = '',$return=false){
		$template = array(
			'touser' => $touser,
		...
		$dataRes = $this->https_request($url, urldecode($json_template));
		$rs = json_decode($dataRes,true);
		if($return==true){
			return $rs;
		}
		if (isset($rs['errmsg'])&&$rs['errmsg']=='ok') {
			return true;
		} else {
			return false;
		}
	}
	function getWinXinUser($openid){
		$access_token = self::getAccessToken();
		$url = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$access_token.'&openid='.$openid.'&lang=zh_CN';
		$dataRes = $this->httpGet($url);
		return $dataRes;
	}
	...
	/*根据MemberID获取场景二维码*/
	function getQrcode($scene_id,$long='0'){
		$postdata = array();
		if($long=='1'){
			$postdata = array(
		...
		$result = self::https_request($url,json_encode($postdata));
		$jsoninfo = json_decode($result,true);
		$ticket = $jsoninfo['ticket'];
		$url = 'https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.urlencode($ticket);
		$imageInfo = self::downloadImageFromWeiXin($url);
		$filename = $scene_id.".jpg";
		$save_path = 'upload/wxqrcode/';
	...
	*/
	...
	public function reply($msg=array(),$return = false)
	{
		...
		if ($this->encrypt_type == 'aes') { //如果来源消息为加密方式
			...
			$xmldata = str_replace('<![CDATA[0]]>','0',$xmldata);
