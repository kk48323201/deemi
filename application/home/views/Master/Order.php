...
#member-order-nav a{ color:#5d5d5d; float:left; line-height:32px; text-decoration:none;font-size:14px; border-bottom:2px solid #FFFFFF; display:inline-block; width:33%; text-align:center;}
#member-order-nav a.cur{ color:#4c7bad; border-bottom:2px solid #4c7bad;}
.layui-m-layerbtn span[yes]{ color:#4c7bad;}

</style>
<div id="member-order-nav"> <a href="#0">抢单</a> <a href="#1">进行中</a> <a href="#2">已完成</a></div>
...
function getListData(inipage,x){
	...
	var url = "<?=site_url('Master/Order?do=getData')?>";
	...
	$.post(url,postData,function(data){
		...
		$.each(data.rows, function(idx, obj) {
			...
			var OrderUrl = site_url('Master/ServiceOrderDetial/'+obj.Sn);
